...
# Like Destroy Form Object
class Like::DestroyForm
  ...
  def already_liked?
    message = "User: #{User.find(user_id).name} has not liked the Product: #{Product.find(product_id).name}."
    errors.add(:existence, message) unless Like.exists?(user_id: user_id, product_id: product_id)
  end
end
